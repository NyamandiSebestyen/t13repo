...
async function beillesztes() {
    try {
        const client = await MongoClient.connect(url);
        const db = client.db("T13");
        const collection = db.collection("Helsinki");
        const muveletek = await collection.insertMany(HelszinkiEredmenyek);
...
function objektumTombLetrehozasa(tomb) {
    let objektumTomb = [];
    for (let i = 0; i < tomb.length; i++) {
        let aktTomb = tomb[i].split(' ');
        let objektum = {
            helyezes: Number(aktTomb[0]),
            csapatMeret: Number(aktTomb[1]),
            sportAg: aktTomb[2],
            versenySzam: aktTomb[3]
        }
...
const HelszinkiEredmenyek = objektumTombLetrehozasa(eredmenyek);
beillesztes();
